...
3. Срок задачи.
4. Обводка узла:
    * Синяя обводка  - задача в работе;
    * Синяя обводка с кружком в углу  - текущая задача в работе;
    * Красная обводка - задача прекращена;
    * Зеленая обводка - задача выполнена;
...
3. Добавлен новый модуль **Tree**. В нем реализовано API **GetTreeStructureActionItemExecutionTask**, которое передает данные для постороения дерева в стороннем контроле.
4. В папке remote_component-tree-view содеражатся исходники стороннего контрола. За основу был взят пример [стороннего контрола](https://github.com/DirectumCompany/sungero-remote-component-example-react).
...
4.	Более подробно про варианты расширения функциональности можно изучить в документации библиотеки [react-d3-tree](https://www.npmjs.com/package/react-d3-tree).
